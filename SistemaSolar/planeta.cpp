...
Planeta::Planeta(float distanceFromSun, float orbitTime, float rotationTime, float radius, GLuint textureHandle)
{
...
void Planeta::calculatePosition(float time)
{
...
void Planeta::getPosition(float* vec)
{
...
float Planeta::getRadius(void)
{
	return radius;
}

// add a moon to this planet
void Planeta::addMoon(float distanceFromPlanet, float orbitTime, float rotationTime, float radius, GLuint textureHandle)
{
	moons.push_back(Luna(distanceFromPlanet, orbitTime, rotationTime, radius, textureHandle));
}

Luna Planeta::getMoon(int index) {
	return moons[index];
}

float* Planeta::getActualPosition() {
	float vec[3];
	vec[0] = position[0] * distanceScale;
	vec[1] = position[1] * distanceScale;
	vec[2] = position[2] * distanceScale;
