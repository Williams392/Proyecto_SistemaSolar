...
void addMoon()
{
	...
	sistemaSolar.addLuna(planetSelected,
		(500 + random(1500)) * sistemaSolar.getRadioDelPlaneta(planetSelected),
		10 + random(100), 0.5 + random(20),
		sistemaSolar.getRadioDelPlaneta(planetSelected) * (0.05f + random(0.2f)), moon->getTextureHandle());
}
...
void init(void)
{
	...
	// Load all image data
	stars = new TGA("imagenes/stars.tga");
	moon = new TGA("imagenes/moon.tga");

	TGA* sun = new TGA("imagenes/sun.tga");
	TGA* earth = new TGA("imagenes/earth.tga");


	// Add all the planets with accurate data. Distance measured in km, time measured in earth days.
	sistemaSolar.addPlaneta(0, 1, 500, .8 * 69500, sun->getTextureHandle()); // sun
	sistemaSolar.addPlaneta(149600000, 365, 1, 13 * 5000, earth->getTextureHandle()); // earth
	sistemaSolar.addLuna(1, 11 * 7000000, 27.3, 60, 6 * 10000, moon->getTextureHandle()); // test moon for the earth
...
void display(void)
{
	...
	sistemaSolar.calcularPosiciones(_time);
	
	if (controls.forward) camara.forward();		if (controls.backward) camara.backward();
	if (controls.left) camara.left();			if (controls.right) camara.right();
	if (controls.yawLeft) camara.yawLeft();		if (controls.yawRight) camara.yawRight();
	if (controls.rollLeft) camara.rollLeft();	if (controls.rollRight) camara.rollRight();
	if (controls.pitchUp) camara.pitchUp();		if (controls.pitchDown) camara.pitchDown();
	...
	sistemaSolar.renderizar();
	glDisable(GL_LIGHTING);

	// possibly render orbits
	if (showOrbits)
		sistemaSolar.renderizarOrbitas();
...
void keyDown(unsigned char key, int x, int y)
{
	...
	case 'o':
		...
		break;
	case ',':
		camara.slowDown(); // slow down camera
		break;
	case '.':
		camara.speedUp(); // speed up camera
		break;
